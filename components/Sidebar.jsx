    ...
    transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
        <div>
            <div className={`flex ${expand ? "flex-row gap-10 " : "flex-col items-center gap-8 "}`}>
                ...
                rounded-lg cursor-pointer'>
                    ...
                    <div className={`absolute w-max ${expand ? "left-1/2 -transalte-x-1/2 top-12": "-top-12 left-0"}
      ...
      <div>
        ...
        className={`flex items-center ${expand ? 'hover:bg-white/10 rounded-lg':
          'justify-center w-full'} gap-3 text-white/60 text-s p-2 mt-2 cursor-pointer`}>
            {
              user ? <UserButton/>
